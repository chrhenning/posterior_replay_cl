...
class WRN(Classifier):
    ...
    def has_bias(self):
        ...
        # FIXME Should we display a warning to notify the user that there are
        # conv 1x1 layers without bias terms? Might be that the user expects
        # `layer_weight_tensors` and `layer_bias_vectors` to have the same
        # length if `has_bias` is True.
