...
"""
Hyperparameter-search configuration for SplitMNIST
--------------------------------------------------
...
_BASE_CONDITIONS = [
    ...
    ({'use_adam': [True]}, {'use_rmsprop': [False],
                            'use_adadelta': [False],
                            'use_adagrad': [False],
    # Momentum option is not used by Adam.
                            'momentum': [-1]}),
    ({'use_rmsprop': [True]}, {'use_adam': [False],
                               'use_adadelta': [False],
                               'use_adagrad': [False]}),
    ({'use_adadelta': [True]}, {'use_rmsprop': [False],
                               'use_adam': [False],
                               'use_adagrad': [False],
    # Momentum option is not used by Adadelta.
                               'momentum': [-1]}),
    ({'use_adagrad': [True]}, {'use_rmsprop': [False],
                               'use_adadelta': [False],
                               'use_adam': [False],
    # Momentum option is not used by Adagrad.
                               'momentum': [-1]}),
    ({'use_adam': [False]}, {'adam_beta1': [-1]}),
    ({'epochs': [-1]}, {'plateau_lr_scheduler': [False]}),
    ({'epochs': [-1]}, {'lambda_lr_scheduler': [False]}),
    ...
    ({'kl_scale': [1.]}, {'kl_schedule': [0]}),
    ({'coreset_size': [-1]}, {'per_task_coreset': [False],
                              'coreset_reg': [1.],
                              'past_and_future_coresets': [False]}),
    ({'calibrate_temp': [False]}, {'cal_temp_iter': [-1],
                                   'cal_sample_size': [-1]}),
]
# Conditions specific to BbB training.
_BBB_CONDITIONS = [
    ({'local_reparam_trick': [False]}, {'disable_lrt_test': [False]}),
    ({'local_reparam_trick': [True]}, {'disable_lrt_test': [True]}),
    ({'mean_only': [True]}, {'kl_scale': [0],
                             'kl_schedule': [0],
                             'regularizer': ['mse'],
                             'local_reparam_trick': [False],
                             'use_prev_post_as_prior': [False],
                             ...
                             'disable_lrt_test': [False],
                             'use_logvar_enc': [False]}),
    ({'radial_bnn': [True]}, {'regularizer': ['mse'],
                              'local_reparam_trick': [False]}),
]
conditions = conditions + _BASE_CONDITIONS + _BBB_CONDITIONS
...
# script.
_SCRIPT_NAME = 'train_split_bbb.py'
...
_ARGPARSE_HANDLE = lambda argv : targs.parse_cmd_arguments( \
    mode='split_mnist_bbb', argv=argv)
