...
#
# title          :probabilistic/prob_mnist/hpsearch_config_split_ssge.py
# author         :ch
...
"""
Hyperparameter-search configuration for SplitMNIST using SSGE
-------------------------------------------------------------
...
:mod:`probabilistic.prob_mnist.train_split_ssge`.
"""
...
####################################
# Conditions specific to SSGE training.
_SSGE_CONDITIONS = [
    ({'heuristic_kernel': [True]}, {'rbf_kernel_width': [1.]}),
    ({'num_ssge_eigenvals': [1]}, {'thr_ssge_eigenvals': [1.]}),
    ({'num_ssge_eigenvals': [5]}, {'thr_ssge_eigenvals': [1.]}),
]
conditions = conditions + hpsplitbbb._BASE_CONDITIONS + _SSGE_CONDITIONS
...
# script.
_SCRIPT_NAME = 'train_split_ssge.py'
...
_ARGPARSE_HANDLE = lambda argv : targs.parse_cmd_arguments( \
    mode='split_mnist_ssge', argv=argv)
